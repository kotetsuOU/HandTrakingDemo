...
* **標準モデルケース**: すべての機能・仕様文書は [Experiments.md](./Docs/Experiments.md) のフォーマットに厳密に拠ること
* **判断優先順位**: 構造とフォーマットの統一 (`Experiments.md` 準拠) > 用語の統一 > 表記の統一 > 仕様の明確化 > 最小限の修正
* **文体・表記**:
...
* **必須構造要素**:
  * **ナビゲーションヘッダー引用ブロック**: 先頭（タイトル `#` 直下）に親ノードリンク `[Wiki.md](./Wiki.md)`、種類タグ (`🏷️ 種類: ...`)、ポータル復帰リンクを含む引用 `>` を配置する
  * **セパレータ**: 大章 (`##`) の直前には必ず `---` (水平線) を配置する
  * **章構成（固定順序）**:
    `## 1. 概要` → `## 2. 設計思想・アーキテクチャ` → `## 3. セットアップ・使用方法` → `## 4. 仕様・パラメータ詳細` → `## 5. デバッグ・留意事項` の5大章順序を厳守する
  * **アーキテクチャ3要素**: 第2章に `2.1 ディレクトリ構成` (ツリー), `2.2 クラス相関図` (Mermaid), `2.3 処理フロー` を含める
  * **セットアップ**: 第3章に `Step 1`, `Step 2` ... 形式の手順、パラメータ表、コード例を含める
  * **折りたたみ構造 (`<details><summary>`) の一般活用**: 数式モデル・理論的背景だけでなく、長大なコード例、サンプルデータ、詳細なトラブルシューティング手順等は `<details><summary>` タグを用いて可読性を維持する
  * **統制ログ連携**: 第5章に `AppLogManager` のサブトリガー一覧と `Logging.md` への相互参照リンクを明記する
