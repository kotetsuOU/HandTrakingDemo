...
### 元パッケージ (Native C++)
* **動作**: 毎フレーム、点群バッファを CPU に Readback（読み戻し）し、C++ DLL 側で KD-Tree や BVH（Bounding Volume Hierarchy）を動的に構築。
* **課題**: 10万点という大規模な点群を扱う場合、GPU → CPU への巨大なメモリ転送と CPU 側での直列的なツリー構築がボトルネックとなり、メインスレッドを数ミリ秒〜数十ミリ秒ブロックしてしまう問題がありました。
...
### 🔴 割愛・削除した点 (Omitted)
1. **Union-Find アルゴリズムによるクラスタ形成**: グラフの連結成分を辿る再帰処理はGPUと相性が悪いため廃止。
2. **クラスタの強制分割 (ExtentSplit)**: 大きくなりすぎた面を強制分割する処理。楕円STMで十分表現できるため不要と判断。
3. **ツリーベースの空間分割 (BVH / KD-Tree)**: ツリーのトラバースはGPU効率を落とすため、空間ハッシュ配列に変更。
...
#### ネイティブ実装：Union-Find グラフ連結
任意の2点 $\mathbf{p}_i, \mathbf{p}_j$ について、距離が閾値 $d_{thresh}$ 以下の場合にエッジを張り、クラスタを結合します。
$$
\|\mathbf{p}_i - \mathbf{p}_j\|^2 \leq d_{thresh}^2 \implies \text{Union}(i, j)
$$
物理的な繋がりは厳密に解析できますが、完全なGPU並列化が不可能であり、CPUでの直列計算が避けられません。
