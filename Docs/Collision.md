...
- **`2` (内部・貫通)**: 
  レイキャストが奇数回（メッシュ内）。凹形状・スキンメッシュを含め、完全に貫通しているとみなし、ハプティクス提示の対象外（除外）とします。
...
1. **空間座標の量子化**: 各点の座標を指定した分解能（例: `cellSize = 0.02m`）で量子化し、3Dグリッド座標（位置のサイコロ）を求めます。
2. **法線方向の量子化 (Normal Bin)**: 各点の法線ベクトル（メッシュ表面からの向き）を、空間の 6 軸（+X, -X, +Y, -Y, +Z, -Z）のいずれかに分類（法線ビン）します。
3. **ハイブリッド・ハッシュ生成**: 「3Dグリッド座標」と「法線ビン（0〜5）」を組み合わせて1次元のハッシュキーを生成し、固定長のクラスタバッファ（例: 1024個）にマッピングします。
4. **並列蓄積 (`AccumulateClusters`)**: 同一のハッシュキー（同じ場所かつ同じ向きの面）に該当する点群は、`InterlockedAdd` を用いて「要素数」「座標の合計値」「法線の合計値（固定小数点）」を並列に足し合わせます。
5. **重心と平均法線の算出**: 最終的に `合計 / 要素数` を計算し、その接触エリアの高精度な「重心座標」と「平均法線方向」を算出します。
...
- **テキストラベル**: 重心の上に `ID:〇 Age:〇` を表示し、トラッカーによるIDの固定化と寿命を目視確認できるようにします。
...
sequenceDiagram
    ...
    Pipeline->>Tracker: Update(centroids, normals)
    Note over Tracker: 前フレームのクラスタと照合し、<br/>ID・Age(安定度)・法線方向を更新
    Tracker-->>Pipeline: TrackedCluster リスト

    alt デバッグ時
        Pipeline->>Pipeline: OnDrawGizmosSelected() で接触重心・ID・Age・法線ベクトルを描画
    else 実機接続時
