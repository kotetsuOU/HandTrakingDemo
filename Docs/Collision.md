...
算出した「最短距離 $d$」と「レイキャスト交差回数 $k$ による内外判定」を組み合わせ、各点 $\mathbf{p}$ の状態を3値に分類します。表面からの閾値を $T_{surf}$ (`surfaceDistanceThreshold`)、裏面（内部）からの閾値を $T_{back}$ (`backfaceDistanceThreshold`) とした場合、状態判定関数 $f_{collide}(\mathbf{p}, d, k)$ は以下のように定義されます。

$$
f_{collide}(\mathbf{p}, d, k) = 
\begin{cases}
1 & \text{if } d \le T_{surf} \text{ and } (k \equiv 0 \pmod 2) \quad (\text{外部からの表面接触}) \\
1 & \text{if } d \le T_{back} \text{ and } (k \equiv 1 \pmod 2) \quad (\text{内部からの裏面接触}) \\
2 & \text{if } d > T_{back} \text{ and } (k \equiv 1 \pmod 2) \quad (\text{内部・貫通}) \\
0 & \text{otherwise} \quad (\text{外部})
\end{cases}
$$

- **`0` (外部)**: レイキャストが偶数回（メッシュ外）であり、かつメッシュ表面からの距離が閾値よりも遠い状態。
- **`1` (表面接触)**: メッシュの外部または内部に関わらず、表面からの距離が閾値内にあり接触が成立している状態。ハプティクス提示（クラスタリング）の対象。
- **`2` (内部・貫通)**: レイキャストが奇数回（メッシュ内）かつ閾値より深くめり込んでいる状態。完全に貫通しているとみなし、提示対象外。
