...
> **Note**
> 
> 以下はコンパイル済みDLLおよびComputeShaderの解析結果を基にした推定アルゴリズムであり、元実装者の設計意図を完全に保証するものではありません。

本ドキュメントは、センサー（Intel RealSense など）から取得され統合されたリアルタイム点群（Point Cloud）と、Unity 上の仮想オブジェクト（手の動的アニメーションモデル等）に対して、**以下の接触判定およびクラスタリング処理を行っていると推定される**アルゴリズムの詳細をまとめたテクニカルリファレンスです。
