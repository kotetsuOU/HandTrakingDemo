...
2. **内積（Dot）の計算**:
   ```math
   \text{Distance} = \mathbf{V}_{shortest} \cdot \mathbf{N}
   ```
   
3. **符号（Sign）による状態判定**:
   - **$\text{Distance} > 0$（プラス）の場合**:
     点 $\mathbf{P}$ はポリゴンの**外側**にあります。$\text{Distance} \le \text{surfaceDistanceThreshold}$ であれば「表面に接触している」と判定します。
   - **$\text{Distance} < 0$（マイナス）の場合**:
     点 $\mathbf{P}$ はポリゴンの**内側（裏側）**にめり込んでいます。$\text{Distance} \ge -\text{backfaceDistanceThreshold}$ であれば「内部にめり込んで接触している」と判定します。
