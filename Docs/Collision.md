...
1. **ベクトル定義**: 
   - 衝突点の座標を `P`、三角形ポリゴン上で最も `P` に近い点を `C` とします。
   - 点から表面に向かう最短ベクトルは `V_shortest = P - C` となります。
   - 対象ポリゴンの表面が外側を向く正規化された法線ベクトルを `N` とします。
   
2. **内積（Dot Product）の計算**:
   ```text
   Distance = V_shortest ・ N
   ```
   
3. **符号（Sign）による状態判定**:
   - **`Distance > 0`（プラス）の場合**:
     点 `P` はポリゴンの **外側** にあります。`Distance <= surfaceDistanceThreshold` であれば「表面に接触している」と判定します。
   - **`Distance < 0`（マイナス）の場合**:
     点 `P` はポリゴンの **内側（裏側）** にめり込んでいます。`Distance >= -backfaceDistanceThreshold` であれば「内部にめり込んで接触している」と判定します。
